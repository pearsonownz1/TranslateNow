...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        // Construct the metadata using the fetched matter_id
        const documentData: any = {
            name: `Evaluation - ${originalFilename}`,
            type: "upload",
            category: "Evaluation", // Keep category
            matter_id: matterIdForUpload.toString() // Send stored matter_id as string
        };

        const metadata = { document: documentData };
        const metadataString = JSON.stringify(metadata);
        console.log("Metadata JSON being sent:", metadataString);
        clioFormData.append('data', metadataString);
        ...
        console.log("Successfully uploaded evaluation to Clio:", clioUploadResult?.data?.id);
