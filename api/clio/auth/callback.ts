...
// --- Basic Validation ---
if (!supabaseUrl || !supabaseServiceRoleKey || !cookieSecret || !clioClientId || !clioClientSecret) {
    console.error("One or more required environment variables are missing for Clio callback.");
}
...
async function getClioUserId(accessToken: string): Promise<string | null> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`Clio API Error fetching user ID: ${response.status} - ${errorData?.error?.message || response.statusText}`);
            return null;
        }
 ...
 */
async function createClioCustomAction(accessToken: string, targetUrl: string): Promise<boolean> {
    ...
    try {
        ...
        if (response.status === 201) { // 201 Created
            ...
            return true;
        } else if (response.status === 422) { // 422 Unprocessable Entity (often means it already exists with same target_url/ui_reference)
             console.warn("Clio Custom Action might already exist (received 422). Assuming success.");
            ...
            const errorData = await response.json().catch(() => ({}));
            console.error(`Failed to create Clio Custom Action: ${response.status} - ${JSON.stringify(errorData)}`);
            return false;
        }
    } catch (error) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const clearCookie = cookie.serialize('clio_oauth_state', '', { /* ... options ... */
        httpOnly: true, secure: process.env.NODE_ENV !== 'development', path: '/', expires: new Date(0), sameSite: 'lax',
    });
    res.setHeader('Set-Cookie', clearCookie);

    // Validate Input
    if (!code || typeof code !== 'string') { return res.status(400).json({ error: 'Missing or invalid authorization code.' }); }
    if (!receivedState || typeof receivedState !== 'string') { return res.status(400).json({ error: 'Missing or invalid state parameter.' }); }
    if (!signedStateFromCookie) { return res.redirect(302, `/dashboard/integrations?clio_status=error&clio_error=${encodeURIComponent('Session state cookie missing. Please try connecting again.')}`); }
    if (!cookieSecret) { return res.status(500).json({ error: 'Internal server configuration error (state secret).' }); }

    // Verify State Cookie (CSRF Protection)
    const originalState = unsign(signedStateFromCookie, cookieSecret);
    if (!originalState) { return res.redirect(302, `/dashboard/integrations?clio_status=error&clio_error=${encodeURIComponent('Invalid session state cookie. Please try connecting again.')}`); }
    if (originalState !== receivedState) { return res.redirect(302, `/dashboard/integrations?clio_status=error&clio_error=${encodeURIComponent('State mismatch (potential CSRF). Please try connecting again.')}`); }

    // --- Retrieve User ID using State ---
    let userId: string | null = null;
    try {
        console.log(`Clio Callback: Validated state ${receivedState}. Looking up user ID...`);
        ...
        if (stateError || !stateData?.user_id) {
            console.error(`Clio Callback: Failed to find user ID for state ${receivedState}:`, stateError);
            throw new Error("Invalid or expired authorization state. Please try connecting again.");
        }

        userId = stateData.user_id;
        console.log(`Clio Callback: Found user ID ${userId} for state ${receivedState}.`);
        ...
        if (deleteError) {
            ...
            console.warn(`Clio Callback: Failed to delete used OAuth state ${receivedState}:`, deleteError);
        }

    } catch (error) {
        console.error("Clio Callback: Error retrieving user ID from state:", error);
        const message = error instanceof Error ? error.message : "Failed to validate authorization state.";
    ...
    try {
        ...
        const tokenResponse = await fetch(tokenUrl, { /* ... fetch options ... */
             method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' }, body: tokenParams,
        });
        const tokenData = await tokenResponse.json();

        if (!tokenResponse.ok) { /* ... error handling ... */
             console.error('Clio token exchange error:', tokenData);
             const errorDesc = tokenData.error_description || tokenData.error || `Clio API Error: ${tokenResponse.statusText}`;
             throw new Error(`Failed to exchange authorization code with Clio. ${errorDesc}`);
        }

        const { access_token, refresh_token, expires_in } = tokenData;
        if (!access_token || !refresh_token || typeof expires_in !== 'number') { /* ... error handling ... */
        ...
        if (!clioUserId) {
            ...
            console.error(`Could not fetch Clio User ID for OpenEval user ${userId}. Tokens will be stored without it.`);
            ...
            console.log(`Retrieved Clio User ID: ${clioUserId} for OpenEval user ${userId}`);
        }
        ...
        if (dbError) {
             console.error(`Database error storing Clio integration details for OpenEval user ${userId}:`, dbError);
             // If we failed to store the Clio User ID previously, this error might be because the column doesn't exist yet.
             if (dbError.message.includes('column "integration_user_id" of relation "user_integrations" does not exist')) {
                 console.error("REMINDER: The 'integration_user_id' column needs to be added to the 'user_integrations' table.");
                 throw new Error("Database schema update required. Integration partially connected.");
             }
             throw new Error("Failed to save integration details to database.");
        }

        console.log(`Successfully stored/updated Clio integration details for OpenEval user ${userId}.`);
        ...
        const actionCreated = await createClioCustomAction(access_token, customActionTargetUrl);
        if (!actionCreated) {
            // Log warning, but don't fail the overall connection process
            console.warn(`Failed to automatically create Clio Custom Action for user ${userId}. It may need to be created manually or via a separate process.`);
        ...
        const message = error instanceof Error ? error.message : "An unknown error occurred during Clio connection.";
        const redirectUrl = `/dashboard/integrations?clio_status=error&clio_error=${encodeURIComponent(message)}`;
        res.redirect(302, redirectUrl);
    }
}
