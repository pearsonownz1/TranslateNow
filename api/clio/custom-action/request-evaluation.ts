...
if (!supabaseUrl || !supabaseServiceRoleKey) {
    console.error("Supabase environment variables are missing for Clio custom action.");
...
interface ClioSubjectData {
    ...
    parent?: { // For documents, represents the Matter, Contact, or Folder it's linked to
        id: number | string;
...
async function findMatterIdRecursive(
    ...
    console.log(`[findMatterIdRecursive depth ${depth}] Checking item: Type=${currentItem.type}, ID=${currentItem.id}`);

    if (currentItem.type === 'Matter') {
        console.log(`[findMatterIdRecursive] Found Matter directly: ID=${currentItem.id}`);
    ...
    if (currentItem.type !== 'Folder' || depth >= maxDepth) {
        console.log(`[findMatterIdRecursive] Not a Folder or max depth reached. Stopping search.`);
    ...
    console.log(`[findMatterIdRecursive] Item is Folder ${folderId}. Fetching its parent...`);
    const folderDetailsUrl = `${clioApiBaseUrl}/api/v4/folders/${folderId}?fields=parent{id,type}`;

    try {
        const folderRes = await fetch(folderDetailsUrl, { headers: { 'Authorization': `Bearer ${accessToken}`, 'Accept': 'application/json' } });
        if (!folderRes.ok) {
            console.error(`[findMatterIdRecursive] Failed to fetch Folder ${folderId} details (${folderRes.status})`);
            return null;
        }
        ...
        if (!parent || !parent.id || !parent.type) {
            console.warn(`[findMatterIdRecursive] Folder ${folderId} has no parent information.`);
            return null;
        }

        // Recurse with the parent
        return await findMatterIdRecursive(parent, accessToken, depth + 1, maxDepth);

    } catch (folderFetchErr: any) {
        console.error(`[findMatterIdRecursive] Error fetching Folder ${folderId} details: ${folderFetchErr.message}`);
        return null;
    }
}
...
async function validateNonceAndFetchSubject(
    ...
    nonce: string,
    accessToken: string
): Promise<{ valid: boolean; data?: ClioSubjectData; error?: string }> { // Reverted return type
    ...
    try {
        ...
        if (response.ok) {
            ...
            try {
                ...
                console.log("Nonce validation successful. Response data (if any):", JSON.stringify(responseData));
            } catch (e) {
            ...
            return { valid: true, data: responseData?.data as ClioSubjectData | undefined };
        } else if (response.status === 403) {
            console.error("Nonce validation failed (403 Forbidden). Nonce might be invalid, expired, or reused.");
            return { valid: false, error: "Invalid or expired request link (nonce validation failed)." };
        } else { // Handle other errors (like the 400 Bad Request)
            const errorData = await response.json().catch(() => ({}));
            console.error(`Nonce validation API call failed: ${response.status} - ${errorData?.error?.message || response.statusText}`);
            return { valid: false, error: `Failed to validate request with Clio (Status: ${response.status}).` };
        }
    } catch (error) {
        console.error("Network error during nonce validation:", error);
        return { valid: false, error: "Network error communicating with Clio for validation." };
    }
}
 ...
 */
async function createOpenEvalQuoteRequest(details: {
...
}, dbClient: SupabaseClient): Promise<{ success: boolean; quoteId?: string; error?: string }> {
    console.log("Attempting to create OpenEval quote request with details:", { ...details, clioMatterId: details.clioMatterId ?? 'NULL' }); // Log null explicitly
    ...
    if (error) {
        ...
        return { success: false, error: "Failed to create quote request in database." };
    }
    if (!data?.id) {
        console.error("Quote request created but failed to retrieve ID.");
        return { success: false, error: "Failed to confirm quote request creation." };
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    console.log(`Query Params - Clio User ID: ${clioUserId}, Encoded Subject URL: ${encodedSubjectUrl}, Nonce: ${nonce}`);

    // Validate required parameters
    if (!clioUserId || typeof clioUserId !== 'string') {
        return res.status(400).json({ error: "Missing or invalid 'user_id' query parameter." });
    }
    if (!encodedSubjectUrl || typeof encodedSubjectUrl !== 'string') {
        return res.status(400).json({ error: "Missing or invalid 'subject_url' query parameter." });
    }
    if (!nonce || typeof nonce !== 'string') {
        return res.status(400).json({ error: "Missing or invalid 'custom_action_nonce' query parameter." });
    }
    ...
    try {
        ...
        console.log(`Looking up integration details for Clio User ID: ${clioUserId}`);
        ...
        if (dbFetchError || !integrationData) {
            console.error(`Failed to find integration credentials for Clio User ID ${clioUserId}:`, dbFetchError);
            // Check if the error is because the column doesn't exist
             if (dbFetchError?.message.includes('column "integration_user_id" of relation "user_integrations" does not exist')) {
                 console.error("REMINDER: The 'integration_user_id' column needs to be added to the 'user_integrations' table.");
                 return res.status(500).json({ error: "Integration setup incomplete (DB schema). Please contact support or reconnect." });
             }
            return res.status(404).json({ error: "Integration not found or not configured correctly for this Clio user." });
        }

        const internalUserId = integrationData.user_id;
        console.log(`Found OpenEval User ID: ${internalUserId} for Clio User ID: ${clioUserId}`);
        ...
        try {
            ...
            console.error(`Failed to decrypt token for user ${internalUserId}:`, decryptionError);
            return res.status(500).json({ error: "Failed to process integration credentials." });
        }

        // --- Validate Nonce (and potentially get basic data) ---
        const validationResult = await validateNonceAndFetchSubject(subjectUrlRelative, nonce, accessToken);

        if (!validationResult.valid) {
            return res.status(403).json({ error: validationResult.error || "Request validation failed." });
        }
        ...
        try {
            ...
            let fields = 'id,etag'; // Base fields
             if (subjectUrlRelative.includes('/matters/')) {
                 fields += ',display_number,description,client{id,etag,name,email_addresses}';
            ...
            if (!detailsResponse.ok) {
                 throw new Error(`Failed to fetch subject details (Status: ${detailsResponse.status})`);
            }
            const detailsData = await detailsResponse.json();
            subjectData = detailsData.data as ClioSubjectData;
            console.log("Successfully fetched subject details:", JSON.stringify(subjectData, null, 2));

        } catch (detailsError: any) {
             console.error(`Error fetching subject details after nonce validation: ${detailsError.message}`);
        ...
        if (!subjectData) {
             ...
             return res.status(500).json({ error: fetchDetailsError || "Failed to retrieve subject data object after validation." });
        }
        ...
        if (subjectUrlRelative.includes('/matters/')) {
            subjectType = 'matter';
            subjectDescription = subjectData.description || subjectData.display_number;
            contactName = subjectData.client?.name;
            primaryEmail = subjectData.client?.email_addresses?.find(e => e.primary)?.email;
                ...
                matterIdToStore = subjectData.id;
                 ...
                 if (!isNaN(matterIdNum)) {
                     matterIdToStore = matterIdNum;
                      ...
                      console.error(`Subject Matter ID is not a parseable number: ${subjectData.id}`);
                 }
            } else {
                 console.error(`Unexpected type for subject Matter ID: ${typeof subjectData.id}`);
            }
            if (matterIdToStore) console.log(`Subject is Matter, using ID: ${matterIdToStore}`);
            ...
            console.log(`Inspecting subjectData.parent: ${JSON.stringify(parentInfo)}`);

            if (parentInfo && parentInfo.id && parentInfo.type) {
                 console.log(`Starting recursive search for Matter ID from parent: Type=${parentInfo.type}, ID=${parentInfo.id}`);
                 matterIdToStore = await findMatterIdRecursive(parentInfo, accessToken);
                 if (matterIdToStore) {
                     console.log(`Successfully found Matter ID ${matterIdToStore} via recursive search.`);
                 } else {
                     console.warn(`Recursive search did not find an associated Matter ID starting from parent Type=${parentInfo.type}, ID=${parentInfo.id}`);
                 }
            } else {
                 console.warn(`Document ${documentId} has no parent information provided by the API.`);
        ...
        if (!matterIdToStore) {
              console.warn(`Proceeding to create quote request without a determined Clio Matter ID for subject ${subjectData.id} (Type: ${subjectType})`);
         }
         ...
         const quoteResult = await createOpenEvalQuoteRequest(quoteDetails, supabaseAdmin);

         if (quoteResult.success) {
            console.log(`Successfully initiated OpenEval quote request: ${quoteResult.quoteId}`);
            // Respond with success - maybe a simple HTML page?
            // For now, JSON response. Could redirect to a success page later.
            res.status(200).json({ message: `Quote request created successfully (ID: ${quoteResult.quoteId}). You can close this tab.` });
        } else {
            console.error(`Failed to create OpenEval quote request: ${quoteResult.error}`);
            res.status(500).json({ error: quoteResult.error || "Failed to create internal quote request." });
        }

    } catch (error) {
        console.error("Unhandled error processing Clio Custom Action GET request:", error);
        const message = error instanceof Error ? error.message : "An unknown error occurred.";
        res.status(500).json({ error: `Failed to process custom action: ${message}` });
    }
}
